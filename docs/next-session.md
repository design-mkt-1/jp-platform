...
right and does nothing is on target; a control that renders 3px taller than its Figma frame is not.
...
- **The dead-node guard**, `src/lib/__tests__/figma-dead-nodes.test.ts`, generated by
  `node scripts/dead-nodes.mjs` from the ledger. It fails on any *new* citation of a deleted Figma
  node and baselines the 182 that were already there. Seen to fail twice before being trusted.
- **The nine `text-[Npx]` line-heights**, read from the design rather than guessed. See the commit.
- Two dead examples removed from `CLAUDE.md`, one dead pointer corrected in `session-11-plan.md`.
...
project path, so a fresh clone has none until `npm run plugins:install`. Orca worktrees and the
Figma connector are per machine too. `Jackpot Figma.fig` was dropped in the repo root on this
machine and is **not** committed; it is also not readable by any tool here — the Figma tools address
the file on Figma's servers by key, not on disk.

**Two traps this session paid for again.** `resize_window` reported success at 390 while
`innerWidth` stayed 430, so viewports are set with Playwright and `innerWidth` is asserted before any
number is trusted. And the accessibility export wrote 239 files into the shared `.next` on top of
`.next-a11y`'s 187 — stop `next dev` before running it, delete `.next` afterwards.
